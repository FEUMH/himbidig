...
// Handle form submission for updating pengurus details
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (in_array($image_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($image_tmp_name, $image_folder)) {
                // Update the pengurus details including the new image
                $conn->query("UPDATE pengurus SET name='$name', position='$position', image='$unique_image_name' WHERE id=$id");
            } else {
        ...
        $conn->query("UPDATE pengurus SET name='$name', position='$position' WHERE id=$id");
    }
